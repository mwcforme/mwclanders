...
export default function AdminLeads() {
  ...
  const loadLeads = useCallback(async () => {
    ...
    let query = supabase
      .from("lead_captures")
      .select(
        "id,created_at,name,email,phone,service,location,source,crm_status,crm_contact_id,crm_error,page_url,tags,attribution"
      )
    ...
    if (result.ok) {
      setRows((prev) =>
        prev
          ? prev.map((r) =>
              r.id === lead.id
                ? { ...r, crm_status: "synced", crm_error: null }
                : r
            )
          : prev
      );
  ...
  const handleResyncAll = async () => {
    const unsynced = (rows ?? []).filter((r) =>
      ["failed", "pending"].includes(r.crm_status)
    );
  ...
  const filtered = (rows ?? []).filter((r) => {
    if (!q) return true;
    const hay =
      `${r.name ?? ""} ${r.email ?? ""} ${r.phone ?? ""} ${r.service ?? ""} ${r.location ?? ""}`.toLowerCase();
  ...
  const unsyncedCount = (rows ?? []).filter((r) =>
    ["failed", "pending"].includes(r.crm_status)
  ).length;
      ...
      {unsyncedCount > 0 && (
        <div className="mb-4 flex items-center justify-between rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3">
          <div className="flex items-center gap-2 text-amber-300 text-sm font-semibold">
            <AlertCircle size={15} />
            {unsyncedCount} lead{unsyncedCount !== 1 ? "s" : ""} not synced to
            GHL
          </div>
          <button
            type="button"
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {filtered.map((r) => {
                ...
                const canResync = ["failed", "pending", "partial"].includes(
                  r.crm_status
                );
                return (
                  <tr
                  ...
                  >
                    ...
                    <Td>
                      ...
                      {r.crm_error && (
                        <div
                          className="mt-1 max-w-[200px] truncate text-xs text-red-300"
                          title={r.crm_error}
                        >
                          {r.crm_error}
                        </div>
