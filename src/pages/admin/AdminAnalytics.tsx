...
export default function AdminAnalytics() {
  ...
  const bookingRate = stats && stats.totalLeads > 0
    ? ((stats.bookedLeads / stats.totalLeads) * 100).toFixed(1)
    : null;

  const completionRate = stats && stats.totalLeads > 0
  ...
  return (
    <AdminLayout title="Analytics">
      ...
      <div className="mb-6">
        <h3 className="mb-3 text-sm font-semibold uppercase tracking-widest text-white/50">Analytics Platforms</h3>
          ...
          {ANALYTICS_TOOLS.map((tool) => {
            ...
            return (
              <a
                ...
                className="flex flex-col gap-3 rounded-xl border border-white/8 bg-[#070B1F] p-5 transition-colors hover:border-white/20 hover:bg-white/5 group"
              >
      ...
      <div className="mb-6 rounded-xl border border-white/8 bg-[#070B1F] p-6">
        <h3 className="mb-3 text-sm font-semibold uppercase tracking-widest text-white/50">GA4 & Clarity Setup</h3>
        <div className="space-y-3 text-sm text-white/70">
          <p>
            <span className="font-semibold text-white">Google Analytics 4:</span> The GA4 measurement ID is injected
            via <code className="rounded bg-white/8 px-1.5 py-0.5 text-xs font-mono text-white/80">VITE_GA_MEASUREMENT_ID</code>.
            Verify events in <span className="text-white/50">GA4 → Reports → Realtime</span>.
          </p>
          <p>
            <span className="font-semibold text-white">Microsoft Clarity:</span> Session recordings and heatmaps are
            configured via the Clarity script tag. View recordings at{" "}
            <a
          ...
          <p>
            <span className="font-semibold text-white">Conversion funnel:</span> Track{" "}
            <code className="rounded bg-white/8 px-1.5 py-0.5 text-xs font-mono text-white/80">lead_capture</code> →{" "}
            <code className="rounded bg-white/8 px-1.5 py-0.5 text-xs font-mono text-white/80">appointment_booked</code>{" "}
      ...
      <div className="mb-6">
        <h3 className="mb-3 text-sm font-semibold uppercase tracking-widest text-white/50">Conversion Metrics (Supabase)</h3>
          ...
          <>
            ...
            <div className="mb-4 grid grid-cols-2 gap-4 md:grid-cols-4">
              {[
                ...
                <div key={label} className="rounded-xl border border-white/8 bg-[#070B1F] p-5">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {/* By location */}
              <div className="rounded-xl border border-white/8 bg-[#070B1F] p-5">
                <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-white/40">Leads by Location</h4>
                    ...
                    return (
                      <div key={location}>
                        <div className="mb-1 flex items-center justify-between text-xs">
                          <span className="text-white/80 truncate max-w-[200px]">{location}</span>
              ...
              {/* By source */}
              <div className="rounded-xl border border-white/8 bg-[#070B1F] p-5">
                <h4 className="mb-3 text-xs font-semibold uppercase tracking-wider text-white/40">Leads by Source</h4>
                <div className="space-y-2">
                  {stats.leadsBySource.map(({ source, count }) => {
                    ...
                    return (
                      <div key={source}>
                        <div className="mb-1 flex items-center justify-between text-xs">
                          <span className="text-white/80 truncate max-w-[200px]">{source}</span>
                          <span className="text-white/50 ml-2 shrink-0">{count}</span>
                        </div>
                        <div className="h-1 w-full rounded-full bg-white/8">
                          <div
