/**
 * LocationSelector — radio group for clinic location in TRT hero form.
 * Includes hover highlight state.
 */
...
export const LocationSelector = ({ value, onChange, error }: LocationSelectorProps) => {
  ...
  return (
    <div role="radiogroup" aria-label="Select center location" aria-required="true">
        ...
        {LOCATIONS.map(({ key, label }) => {
          ...
          return (
            <label key={key} style={{
              ...
              <input type="radio" name="hf-location" value={key} checked={sel}
      ...
      <style>{`label:has(input[name="hf-location"]:focus-visible){outline:2px solid ${ORANGE};outline-offset:2px}`}</style>
    </div>
  );
};
